...
def morse_to_eng_puts "Enter signal durations, use a signal longer than 4 second (4000 milis) to differentiate between letters, finish with '.' " # Gives short instruction on what must be done for the function to work
  ...
  puts morse_to_eng(morse,engltr)                       # Calls the corresponding main function(morse,engltr)
  str=[]
...
def choice(morse,engltr)
  ...
  if i == 1                                               # Checks answer if 1
    ...
    puts "[1] True morse experience"
    puts "[2] written dots and dashes"
    d= gets.chomp
    if d == 1
      ...
      puts "Press enter to start"
      d=gets.chomp                                          # Creates a stop so that user can prepare
      puts morse_to_eng_true(morse,engltr)                       # Calls the corresponding main function
    elsif d==2
