...
def convert!(morse)
  ...
  i=0                                                     # Creates incrementer
  while i<morse.length                                    # Loop for all characters in all items from array morse 
...
def choice(morse,engltr)
  ...
  puts "[1] convert English to morse code"
  puts "[2] convert morse code to English"
  i = gets.chomp.to_i
  if i == 1
    puts "Write a phrase in all caps"
    eng_to_morse_time(morse,engltr)
  elsif i == 2
    "Enter signal durations use a signal longer than 1 second to differentiate between letters"
    puts morse_to_eng(morse,engltr)
  else
