  ...
  while str[str.length-1]!="."                            #Stops the convertion if the finished string ends with a period
    ...
    while g < 800                                         # Differentiates words
      ...
      if g < 800                                          # Checks wether or not the length is valid as a puls for letter or if it is puls for separating letters
        ...
        str << morse_to_engltr(engltr,convert(h),morse)   # Calls function morse_to_engltr with arguments for the list of English letters and the return of the function convert with argument h aswell as list of morse codes
        p str                                             # prints completed string
      end
    end
