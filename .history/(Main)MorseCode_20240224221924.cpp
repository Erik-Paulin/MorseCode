...
void eng_to_morse_time() {
  ...
  for (unsigned int i = 0; i < input.length(); i++) {
    ...
    for (unsigned int j = 0; j < sizeof(engltr) / sizeof(engltr[0]); j++) {
      if (ch == engltr[j].charAt(0)) { // Check for match in English letters array
...
void choose_conversion() {
  ...
  if (choice == 1) { // Check if

   if (choice == 1) { // Check if user chose option 1
    eng_to_morse_time(); // Call function to convert English to Morse code
  } else if (choice == 2) { // Check if user chose option 2
