...
def eng_to_morse_time(morse,engltr)
  ...
  time=convert!(eng_to_morse(morse,str,engltr))           # Assings return variable with result from convert! function which inturn uses return from eng_to_morse function that uses arguments for list of letters in morse(morse) the  
  return time
end
