...
def morse_to_eng(morse,engltr)
  ...
  while str[str.length-1]!="."                            # Stops the convertion if the finished string ends with a period
    ...
    while g < 0.800                                         # Differentiates words
      start1=Time.now
      g=gets
      stop1=Time.now
      g=(stop1-start1)
      p g                                    # gets puls length
      if g < 0.800                                          # Checks wether or not the length is valid as a puls for letter or if it is puls for separating letters
      ...
      elsif g < 1.5 && g > 1000                                               # Letter done, sets for convertion to string
...
def convert(lttr)
  ...
  lttr.each do |duration|                                 # goes through each item in lttr and converts to either dot or dash
      if duration <= 0.300                                  # when puls duration is shorter than 300 it's a dot
        morse_lttr += "*"                                 # adds dot to return string
      elsif duration > 0.300 && duration < 0.800              # when puls is between 300 and 800 it's a dash
...
def convert!(morse)
  ...
  while i<morse.length                                    # Loop for all items from array morse
    ...
    time << 1.000                                          # Add long puls to differentiate between letters
    i+=1                                                  # Increments i
  end
...
def choice(morse,engltr)
  ...
  if i == 1                                               # Checks answer if 1
    ...
    puts "Enter signal durations, use a signal longer than 1 second (1000 milis) to differentiate between letters, finish with '.' " # Gives short instruction on what must be done for the function to work
